...
# --source ./dataset/pills/images/test/ --iou-thres 0.8
# --conf-thres 0.8
...
#         for row in rows:
#             pill_dict[row[1]] = row[0]
#         return pill_dict
#
#
# res = getPillID('./label.csv')
...
def run(
        ...
        name='exp0408',  # save results to project/name
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        # new_cls = int(res[txt_path.split('/')[-1].split('_')[0]])
...
def run_detection(img_path):
    ...
    detect_dir = run(source=img_path)
    return detect_dir
