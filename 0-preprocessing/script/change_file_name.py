...
folder_path = '/media/wall/4TB_HDD/full_dataset/0423_dataset/capsule2/*.png'
file_path_prefix = '/media/wall/4TB_HDD/full_dataset/0423_dataset/capsule2/'
for file_path in glob.glob(folder_path):
    capsule_id = file_path.split('/')[-1].replace('.png', '').split('_')[0]
    old_file_num = file_path.split('/')[-1].replace('.png', '').split('_')[-1]
    new_file_num = str(int(old_file_num) + 20)
